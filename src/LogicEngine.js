...
export class LogicEngine {
  ...
  tick(dt) {
    ...
    for (const p of this.players) {
      if (p.dead) { p.respawn -= dt; if (p.respawn <= 0) { p.dead = false; p.hp = p.maxHp; p.retreating = false; const f = FOUNTAIN[p.side]; p.pos = { x: f.x, y: f.y }; p.state = "回防"; } continue; }
      p.atkCd -= dt;
      ...
      if (nearFountain) p.hp = Math.min(p.maxHp, p.hp + p.maxHp * 0.20 * dt);
      else if (!foe) p.hp = Math.min(p.maxHp, p.hp + p.maxHp * 0.02 * dt);
      if (foe) {
        foe.hp -= p.power * dt * 0.92 * lateFactor;
