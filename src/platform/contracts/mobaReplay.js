 ...
 *
 * ── 為什麼從 2s 改成 2.5s（2026-08-07）──────────────────────────────────
 * `check_moba_experience26` §17 要求一場重播 < 2 MB。每格約 2.9 KB，
 * 2s 取樣在一場 22 分鐘的對局會產出約 700 格 ≈ 1.97 MB——**98.5% 的上限**。
 * 也就是說任何讓比賽稍微變長的改動都會踩爆它（越塔判斷收緊之後就踩到了）。
 *
 * ⚠ 這裡順帶暴露一個既有的不一致：`MAX_FRAMES = 1200` × 2.9 KB ≈ 3.5 MB，
 *   與「< 2 MB」的斷言互相矛盾，只是基準剛好落在下面才一直沒被發現。
 *   修法是把**取樣密度**調到與位元組上限相稱，不是放寬那個上限：
 *   2.5s ⇒ 同樣長度的對局約 560 格 ≈ 1.6 MB，留出真正的餘裕。
 *   全場仍然完整覆蓋（只是每 2.5 模擬秒一格），播放靠既有的位置線性插值。
 */
export const FRAME_INTERVAL_S = 2.5;
/** frame 數上限（≈ 50 分鐘 @ 2.5s）。到頂停止擷取，replay.truncated = true。 */
