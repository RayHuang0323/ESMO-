...
- **只在四個事件開窗**：出生（開局 tick 0）、復活落地、回城抵達泉水、走路進入泉水範圍。窗在事件當 tick 立即結算，不佔模擬時間。
- 走路進泉水：Owner 於 M1 核准 Q3 建議（與回城抵達同效果），M2 實作為 `fountain` 窗；只在「泉水外 → 泉水內」那一 tick 開，
  復活／回城落地當 tick 已開過窗，不重複開。
- 死亡中不能買。沒有「隔空購買」。
