...
| N2／N3／N4／N5 | **瀏覽器實測（Release 前補齊）**：見下方 §13 |

### §13 N2–N5 消費端 gate（Release 前補齊，2026-09-05）

`tools/browser_check_expansion_consumers.mjs` —— **94/94 PASS**（桌機 1366 ＋ 手機 390）。

**為什麼不是走完整賽事流程**：這三個畫面只有在配對 → 房間 → 場次全部成立之後
才到得了（實測 `startPracticeMatch` 只做到房間，session 要靠 `useMatchFlow` 輪詢）。
Owner 明文不要求跑完整場，也不得為了測試在產品裡加捷徑。

**改用的方式**：`tools/browser/mountConsumer.jsx` 把**真正的元件**掛到游離容器，
搭配**真正的 profileStore**。元件、Store、解鎖判定全部是產品那一份，只有
「怎麼到達這個畫面」被繞過。掛載器住在 `tools/`，產品沒有任何地方 import 它
⇒ **不進 production bundle**（release build 實測 `mountScreen` 命中數 = 0）。

每個節點驗四件事，兩種裝置各一輪：

| | 驗到什麼 |
|---|---|
| ① 解鎖前 | 給足點數但一個節點都沒投入 ⇒ 面板**不存在**（不是隱藏，是沒渲染） |
| ② 購買後 | 走**真實** `purchaseTeamDevelopment()` 買完整條前置鏈 ⇒ 旗標成立、點數正確扣除、面板出現且可見、有實際內容 |
| ③ reload 後 | 旗標仍在、已投入點數不變、面板仍然出現 |
| ④ 版面／錯誤 | 面板不造成水平溢出；面板自己的互動 ≥44×44；console／page errors = 0 |

⚠ **觸控目標的判準只涵蓋面板自己的互動**。整個畫面的既有按鈕
（例如 BanPick 的 210 個英雄格中有 110 個 <44px）是既有內容，**不在本輪 scope**，
gate 只做觀察值回報，不列入 pass/fail —— 既不把別人的既有問題當成本輪回歸，
也不假裝沒看到。這是獨立的 mobile 可用性待辦。
