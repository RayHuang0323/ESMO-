...
### 8.1 第一個 upgrade 落在第 1 天，不是 Week 2–4

規格目標是「第一個 meaningful upgrade 約 Career Week 2–4」。實際上
**起始 1 點在第 1 天就可以投入**——因為那是 TD-56 之前就存在的種子值，
本輪刻意不動它（動它 = 改新存檔開局行為，屬於另一個決策）。

第一個**賺到的**點落在 W2–W3（主線玩法），符合目標的節奏意圖。

> **需要 Owner 裁示**：要不要拿掉起始種子點，讓第一次投入延後到 W2–W3？
> 拿掉的話新存檔第一次進戰隊發展會是「0 點、什麼都不能買」。
