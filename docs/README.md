...
- **任何新的玩家端 UI／UX**：`design/ESMO_UIUX設計原則.md`
  —— 全域設計原則（progressive disclosure、資訊三層級、文字密度、一畫面一重點、
  遊戲感與互動狀態、手機規則、玩家端禁用工程術語、新 UI 前的九問）。
  **建立或大改任何玩家端畫面前必讀**；`AGENTS.md` §UI 呈現原則只放引用。
