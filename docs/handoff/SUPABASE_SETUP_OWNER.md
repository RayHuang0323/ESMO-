...
✅ **`deploy.yml` 已經接好了（B1D.2）**，所以你**只要填這兩個變數**，
下一次部署就會生效——不需要再找我改任何東西。

⚠ 填之前先確認你複製的是 **anon / publishable**：
部署流程會在 build 之前檢查，**貼錯 service-role key 會直接讓部署失敗**
（訊息會說明原因，但**不會**印出 key 本身）。這是刻意的——
`VITE_*` 一旦 build 出去就永久公開，寧可部署失敗也不要把它放出去。

⚠ 只填一個也會讓部署失敗。那種「設定了卻沒作用」的狀態最難查，
所以直接擋下來。要嘛兩個都填，要嘛兩個都留空。

⚠ **正式站不會開匿名登入**（workflow 刻意不傳那個變數）：
訪客帳號換裝置就找不回來，不該讓玩家在正式站用它建立整個生涯。
...
填好 `.env.local` 之後可以直接跑：

```
node tools/check_supabase_env.mjs        # 只檢查設定，不打網路
node tools/check_supabase_remote_e2e.mjs # 真的連線驗證（沒憑證會明確 SKIP）
```

第二支會**真的**在你的 Supabase 專案裡寫入再刪除測試資料，
所以建議先在測試專案上跑。它跑完會告訴你哪幾項驗過、哪幾項跳過——
**跳過的就是跳過，不會被算成通過。**

⚠ 其中 Google 登入那一條**腳本做不到**（需要人在瀏覽器按同意），
會標成跳過，請在正式站手動登入一次確認。
