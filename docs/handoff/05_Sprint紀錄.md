...
| 既有 CS/P0/C2C/C3/C4 gates | **全 PASS** |
...
- 狀態：`C5B_UTILITY_FX_READY_FOR_OWNER_ACCEPTANCE`。下一階段 C5C 不在本輪範圍。
...
- `simulateFps` 原先 route 決策沒有完整消費 phase、score、economy、survival、Bomb、weapon mix 與 map control；現在以 phase tactic + deterministic weighted candidates 選擇 route／execute／rotate／post-plant anchor。
...
- desktop／390px mobile 均由真實賽前 UI 選擇 `f1 → f3 → f5 → f6` 四層配置並觀測到 authoritative `source=pre-match-layout` phase selections。
- 四層賽前 UI 使舊 browser verifier 的入口變動；已改用正式 `data-testid` phase／confirm 入口，這是 verifier compatibility fix，不是繞過 runtime failure。

#### 三、交付邊界

- 未開始 C5C。Node/headless 390px 仍不能取代 Android 真機視覺、FPS、觸控、AudioContext、喇叭與熱節流驗收。
...
- 新增 `tools/check_cs_c5b_route_interrupt.mjs` 與 `tools/check_cs_c5b_route_interrupt_runtime.mjs`。五種 opening／rotate／flank／retreat／post-plant-retake case 均 PASS，cadence/navigation violations `0`。
...
<!-- latest origin/main sprint history retained; candidate V3 history follows -->
...
## MOBA Item System — M4b.5 Tower Safety & Dive Fix（2026-09-16）

分支 `design/moba-items-v1-m0`。依 `docs/design/MOBA_塔下越塔Audit_v1.md` 的根因直接修塔與 AI；**模擬語意 bump 為 `moba-sim.v5`**（指紋 `a47541d98096db2e`）⇒ v1–v4 歷史挑戰不再可重播。報告：`docs/design/MOBA_塔安全性修正_M4b5_v1.md`。

### 一、改動（全在 v3 規則集的 `towerSafetyV1` 開關下；v1/v2 無此鍵 ⇒ 歷史基準逐位元不變）

- 射程同源：`towerRange()` ＝ 碰撞外緣 ＋ 本場最長英雄攻擊距離（路塔 12.7–13.6、門牙塔 12.8、主堡 12.3）；開火／仇恨／AI 塔區／debug／撤離點全讀它。
- 塔傷同源且與 lateFactor 脫鉤：`towerHeroShot 25 × 時間曲線(cap 2.6) × 連續命中增幅`；20 分單發 56（5.8% 最大生命，舊值 455／48%）。
- 塔可以擊殺英雄；擊殺歸屬 8 秒內最後出手的敵方英雄（`_towerKillerFor`），Σk == bK+rK == Σd 仍成立。
- 仇恨：找不到小兵不清英雄鎖定（路塔增幅真的會疊）；「有兵先打兵」改用世界距離（射程內英雄 0 發的死區消失）。
- AI：越塔／風險／塔區退出／撤退門檻改用同一個 `_towerHeroShot()`＋`_towerExposure()`；越塔擊殺時間算入附近隊友；連續吃塔上限只在「有兵扛塔」或「真的拆得動的圍攻」放寬，離開塔區滿 3 秒才歸零；塔區判定同時判**移動目標點**（修掉在射程邊緣每秒吃一發的擺盪）。

### 二、量測（`reports/moba-tower-safety-m4b5/`）

- 射程邊界：射程−0.3 會被打、+0.3 不會（路塔與門牙塔各 3 seeds 一致）。
- 塔擊殺：早期 15 發／8 秒、20 分 9–10 發／5–5.5 秒、25 分 9 發／5 秒；擊殺者正確、KDA 不變量成立。
- AI 越塔（10 seeds）：單人無兵只吃 2–3 發就退（舊版一場 7–24 發）；對殘血目標仍 100% 擊殺，代價 48–51% 血；後期「半血從塔內開始」死亡 30–80%，死因是英雄傷害（塔只佔 5–7.5%）。
- 遠程站在射程外打抱塔守方的 tick：320–472 → **0**。
- income 1.9 重驗（standard 200 seeds）：第一／二／三件 T3 8.95／16.34／20.22、20 分每人 2.38 件、帳務 0／0／0 ⇒ 1.9 不需因塔修正調整。
...
## MOBA Item System — M4c / M4c.1 Combat & Marksman Power（2026-09-16）

分支 `design/moba-items-v1-m0`。基準固定 `moba-sim.v6`、income 1.9、Tower／Siege 不動。
報告：`docs/design/MOBA_射手戰力調整_M4c1_v1.md`。證據：`reports/moba-items-m4c/`。

### 一、M4c 量測（先量再調）

新增兩支離線工具（走引擎同一條傷害路徑，不跑對局）：
`tools/balance/moba_item_power_probe.mjs`（滿裝輸出倍率／有效生存／TTK／逐件跳升）、
`tools/balance/moba_item_power_decompose.mjs`（把戰力拆到屬性家族）。
balance runner 加 `at`（10／15／20／25 分 damage／deaths／kills／等級）與 `spikes`（T3 完成前後 90 秒輸出速率，跳過購買後 20 秒回程）。

配對 200 seeds（同 seed、OFF vs ON＋income 1.9）結果：

| 項目 | OFF | ON |
|---|---|---|
| 時長 中位／P90 | 25.35／31.9 | 23.41／27.37 |
| 擊殺 10/15/20/end | 2.84／6.89／12.99／25.36 | 2.01／6.06／16.46／33.45 |
| 每分鐘傷害倍率（ON/OFF） | — | 戰士 1.50、法師 1.49、**射手 2.46**、坦克 1.22、輔助 1.29、刺客 1.21 |
| 每次死亡承受傷害倍率 | — | 坦克 2.10、輔助 1.53、戰士 1.40、射手 0.77、刺客 0.77、法師 0.65 |

離線滿裝模型：射手 4.01、法師 2.96、刺客 2.00、戰士 1.78、輔助 1.29、坦克 1.12；
有效生存 坦克 2.95、戰士 2.37、輔助 1.85；1v1 鏡像 TTK 8.2（射手）～82.1 秒（坦克），基準 31.1 秒。

⇒ 超標的是**射手輸出**（對局 2.46 / 模型 4.01，目標 1.8–2.1）與坦克生存（2.95，目標約 2.0）。
屬性拆解顯示射手是**三段相乘**（AD −48.9%／攻速 −34.5%／暴擊 −29.4%），其他流派只吃單一主因。

### 二、M4c.1 調整（Owner 指示 A'：只動射手專屬裝備的攻擊鏈）

`itemCatalog.js` 射手族 T3 的暴擊鏈，兩段式下修（AD／攻速／價格／組件／unique／效果全未動）：

| 裝備 | 原始 → 現值 |
|---|---|
| `t3_dawnbow` | cc 0.25 / amp 0.35 → **0.10 / 0.10** |
| `hunter`／`reaper`／`pierce`／`bloodoath`／`rendspear` | cc 0.20 → **0.05** |

每次調整都重跑同一套證據（離線模型 → 屬性拆解 → 配對 200 seeds）。

| 指標 | 原始 | 第一次 | 第二次 |
|---|---|---|---|
| 射手 實戰倍率 | 2.46 | 2.23 | **2.18** |
| 射手 離線模型 | 4.009 | 3.395 | **3.132** |
| 射手 1v1 TTK | 8.23 | 9.72 | **10.54 秒** |
| 戰士／刺客 實戰倍率 | 1.50／1.21 | 1.54／1.18 | **1.56／1.18** |
| 時長 中位／P90（ON） | 23.41／27.37 | 23.82／27.78 | **23.62／26.94** |
| T3 timing | 8.98／16.45／20.25 | 8.97／16.38／20.31 | **8.98／16.38／20.25** |
| 帳務 | 0／0／0 | 0／0／0 | **0／0／0** |

items OFF 三輪數字完全相同（median 25.35／P90 31.9／max 40.85）⇒ OFF 路徑未受影響。

### 三、驗證

| 驗證 | 結果 |
|---|---|
| `check_moba_items_m1` | 69/69 PASS（G11 暴擊基準同步 0.15／1.85；AD 97、攻速 0.25 不變） |
| `check_moba_items_m2` | 53/53 PASS |
| `check_moba_items_m3` | commit 前 65/66（G6 範圍守門擋下刻意修改的 itemCatalog.js），commit 後結果見下一節 |

### 四、未達標／未做

- **射手 2.18 仍高於 1.8–2.1 上緣 0.08**。依指示「達標就停、不擴大範圍」，未再自行第三次調整；建議值與取捨寫在報告 §5。
- 暴擊鏈已近下緣：完全歸零也只到模型 2.98／預估對局 1.84 ⇒ 殘差主要來自 AD 與攻速（含通用鞋），屬另一類參數。
- 未動：法師、坦克、income、Tower／Siege、AI build、side bias、`K_AD`／`K_AP`／`CRIT_BASE`。
- 未 push、未 deploy、未跑瀏覽器（無 UI 改動）。

## MOBA Item System — M4c Closure（2026-09-16）

分支 `design/moba-items-v1-m0`。**本輪程式碼零改動**，只做判定與 audit。
報告：`docs/design/MOBA_裝備平衡收斂_M4c_v1.md`；新增長期守則 `docs/design/MOBA_裝備設計守門_v1.md`。
證據沿用 `reports/moba-items-m4c/off-vs-on-after2/`（配對 200 seeds）與 `power-after2.json`，未重跑。

### 一、離線模型為何系統性高估

`power_probe` 有兩欄輸出倍率，先前只引用了「對無裝備目標」那欄：

| 流派 | outputMult（對無裝備） | outputMultMirror（雙方滿裝） |
|---|---|---|
| 法師 | 2.960 | **2.554** |
| 坦克 | 1.116 | **0.687** |

再加兩個模型涵蓋不到的實戰折扣：第 3 件 T3 在 20.25 分完成、ON 中位時長 23.62 分 ⇒ 滿裝只佔全場約 **14%**；以及模型假設不死。

### 二、法師：實戰未超標，反而低於目標帶

每分鐘傷害 503.1 → 763.9（**1.52**，目標帶 1.8–2.0）；死亡 2.42 → 4.38；有效生存倍率 **0.63**；換命效率 0.77；擊殺 2.79 → 3.81。
⇒ 法師拿裝備後**更脆而不是更強**，把裝備換成擊殺參與。形狀正確，**不需處理**。
⚠ 日後要拉法師，瓶頸是生存與滿裝時間，不是 AP 數值。

### 三、坦克：實戰正常，沒有打不死

有效生存（整場承受傷害／死亡）6980 → 15298（**2.19**，目標約 2.0）；死亡 2.63 → 1.56；輸出倍率僅 1.25；擊殺 2.63 → 1.45。
先前的 2.95 是離線模型（固定攻擊者戰力）；實戰敵方同樣吃到 1.2–2.2× 輸出，抵銷大半。
⇒ 坦克每場仍死 1.56 次、擊殺佔比下降，是前排不是勝負手。**不需處理**。

### 四、24 件 v1 T3 Identity Audit（只 audit，未改數值）

v1 實際為 **8 family × 3 件 = 24 件**。**24/24 都答得出「誰買、為什麼買」，8 族內部三件都構成真實取捨**。
四項風險（詳見報告 §5）：R1 family A 暴擊 identity 被 M4c.1 稀釋（已由 Owner 裁決接受）；
R2 `t3_torrent`／`t3_soulrend` 0 購買（build path 可達性問題，非定位問題）；
R3 `t3_nightscythe`／`t3_warbringer` 無 unique 無 effect，純數值棒；R4 `t3_bulwark` 反暴擊 counter 價值被 R1 連帶貶值。

### 五、新增長期守則

Owner 於本輪訂下《MOBA 裝備設計守門》：每個 T3 family 必須保留明確定位／核心屬性方向／Power Fantasy／同族選擇差異／situational counter 價值；
Balance 優先調效率與幅度；**數值 Gate 與裝備 Identity 衝突時先回報，不得為過 Gate 砍特色**。
M4c.1 射手案即為此流程第一個實例（2.18 記為 ACCEPTED WITH KNOWN DEVIATION）。
...
- 公平性 **n=1000 final validation**：全關 藍 49.6%（rK/bK 0.989、中位 22.9、p95 32.5、最長 42.3、擊殺 15.2）、
  道具＋技能 藍 47.6%（0.977、21.3、28.0、38.3、擊殺 31.3）；兩組各 1000 場**全部正常結束**。
...
### 2026-09-27 RELEASED：feature/moba-lane-jungle-balance（main `cefaccb`，moba-sim.v12）

- fetch：origin/main = `a7afc5c`（Release 1 之後）。原 candidate `e1be5f2` 從 `8bf51df` 開出 ⇒ rebase 到 `a7afc5c`（只有 05／08 兩份文件的檔尾追加衝突，兩邊保留；程式碼無衝突）⇒ `cefaccb`，fast-forward normal push，無 force。
- Deploy：run `36263295855` success；正式 bundle 含 `moba-sim.v12`、`laneSkillV1`、`jungleReachV1`、`smiteContextMaxHpFrac`。
...
- Production smoke（正式站，桌機＋390）：polish r2 35/37、spectacle 40/41。紅燈＝Workshop（DEV-only 頁，正式站本來就沒有）×2，
  以及桌機 D1「取樣 90 秒內沒有英雄倒地」（前置條件不成立；標記可見 0；同 bundle 390 取樣 16 次倒地、標記 0）。
...
- 註：`90f191f` 的四份 handoff 追加內容因 shell 反引號被吃掉而缺字，已於下一個 commit 還原後重寫（本段即重寫版）。
