...
| **Q5** | **跨賽季換季**（S1 → S2 → S3…） | ✅ 已完成，**未 merge／未部署** | `q5` 66/66 |
...
1. **選手老化與世代更替**——年齡曲線、退休、續約、新秀補進。
   Q5 刻意排除它；沒有這個，賽季序列在「經營」意義上還是靜態的。
2. **AI 隊伍隨賽季演進**——七支 AI 目前每季 roster 相同。與第 1 項同一個題目。
3. 既有技術債：舊賽果重送可能寫進別的 fixture（一行修法候選已記錄於 08）。
4. 第二階段（設計文件 §10）：正式「模擬出賽」、模擬器與 LogicEngine 勝率校準、
