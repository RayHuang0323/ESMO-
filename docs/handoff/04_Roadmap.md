...
| **F2.1** | **Competition Fan Award Policy** | ✅ **已實作**（branch `feature/fan-f2-1-award-policy`）。新增 `fanPolicy` 與 `prizePolicy` 平行 ⇒ CS 聯賽、MOBA 巡迴站／年度總決賽都拿得到賽季粉絲且不發現金；policy-less Event 仍 fail-closed。**TD-28 CLOSED**。`check_fan_system` 66/66 |
