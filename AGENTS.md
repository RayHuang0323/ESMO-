...
- **動態是允許且被鼓勵的**，規則見下面〈Motion Policy〉。
...
### Motion Policy（2026-09-02 起，取代「ESMO 盡量不做動畫」的舊共識）

**舊規則作廢。** 過去交接時流傳的「ESMO 避免動畫、能不動就不動」不再適用——
它讓好幾個畫面停在靜態卡片堆，而 Ray 明確要的是**遊戲感**，不是後台表單。
新規則是：**動態預設允許，但必須有理由、必須可關、必須不影響邏輯。**

判準只有一句：**這個動態有沒有在傳達狀態變化？** 有 ⇒ 做；沒有 ⇒ 不要做。

**允許（不需要另外請示）**

| 用途 | 例子 | 建議時長 |
| --- | --- | --- |
| 進場 | 同方向小幅 `opacity + translateY`，卡片之間 stagger | 400–600ms |
| 狀態轉換 | 切換分頁／流派／模式時的換色、重新進場 | 250–650ms |
| 數值變化 | 進度條寬度、刻度亮起、數字 count-up | 300–600ms |
| 可操作提示 | 「可領取」的低頻 pulse、下一格的呼吸 | ≥2s 一循環 |
| 互動回饋 | hover elevation、press `scale(0.95)`、focus ring | 120–200ms |
| 完成回饋 | 領取／解鎖後的一次性光暈，自己結束 | ≤1.5s，**一次** |
| 環境氛圍 | 極慢、極淡的漸層漂移，僅限 hero／場景層 | ≥12s 一循環 |

**禁止**

- 長駐重粒子、常態閃爍、無意義的永久動畫（沒有在說任何狀態的動畫）。
- 動畫期間擋住點擊、延後 state 更新、或讓使用者必須等動畫演完才能操作。
- 手機掉幀：320 / 390px 上動畫不得造成可感知卡頓；動 `width`／`top`／`left`
  這類會觸發 layout 的屬性要先想過，優先用 `transform` 與 `opacity`。
- 為了動畫而改 domain：動畫旗標是**呈現層 state**，不得寫進 Store schema
  或 persistence（例：領取後的慶祝只是 component 內的 `useState` + timeout）。

**必要條件（每個帶動態的畫面都要做到）**

- **`prefers-reduced-motion: reduce` 必須支援**：直接呈現**完成狀態**，
  移除位移、pulse、drift，但**顏色與文字狀態全部保留**——減少動態不是降級資訊。
- 動畫寫在該畫面自己的 CSS 檔（如 `src/screens/manage/clubMastery.css`），
  用 `@keyframes` + class 切換；不要散在 inline style 裡。
- GSAP 只在需要 timeline 編排時才引入（參考 `useDashboardMotion.js` 的 scoped
  `useGSAP`）；單純的進場／hover／pulse 用 CSS 就好，不要為此加依賴。
- 交付時仍要照〈手機優先原則〉列出「未經真機實測」項目（實際 FPS、觸控手感）。

參考實作：`src/screens/dashboard/`（timeline 編排）與
`src/screens/manage/clubMastery.css`（純 CSS，含 reduced-motion 區塊）。
視覺語言與 token 見 `docs/design/ESMO_Design_System_v1.md`。
