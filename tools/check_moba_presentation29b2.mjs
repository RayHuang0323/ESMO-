...
// 8) HP bar 使用真實 hp
{
  const V3D = code(src("src/MobaView3D.jsx"));
  const RS = code(src("src/screens/moba/MobaReplayScreen.jsx"));
  // S29B4：ReplayScreen 白畫面防護把 a.ob?.[i] 改為 a?.ob?.[i]（多一層 optional chaining）
  //   ——仍是「讀 frame.ob 真實 hp」，語意不變，斷言隨之放寬 a\??\.ob。
  // ⚠ S29B6 更新本條斷言的 3D 部分：舊斷言綁 `/clamp\(ob\.hp/`，對應舊寫法
  //   `shownHp = lerp(shownHp, hpNow, dt*6)`（**追值**：顯示值一路追真值，永遠落後）。
  //   29B6 D 項查出那正是「血條還剩兩三成、模型卻突然消失」的成因之一：追值殘留 +
  //   死亡瞬間強制歸零。現在目標 hp 改成與英雄**同一條 prev→snapshot 插值時間軸**
  //   （lerp(pv.hp, nx.hp, a)；死亡 tick 走 lerp(pv.hp, 0, a) ⇒ 血條歸零與死亡同幀）。
  //   兩者都「只吃 snapshot.objectives 的真實 hp、不造假血條」——這是本條真正要守的
  //   紅線，故斷言改綁新的真值插值式，語意不變且更嚴（不再容許追值殘留）。
  ck("8) HP bar 使用真實 hp（3D：objMap←snapshot.objectives、hp 由 prev→next 真值插值；小兵縮放吃 m.hp；Replay bar 讀 frame.ob；無假血條）",
    /objMap\[/.test(V3D) && /snapshot\.objectives/.test(V3D) && /prev\.objectives/.test(V3D) &&
    /lerp\(pv\.hp, nx\.hp, a\)/.test(V3D) && /lerp\(pv\.hp, 0, a\)/.test(V3D) &&
    /clamp\(hpNow, 0, 1\)/.test(V3D) && /nm\?\.hp \?\? m\.hp/.test(V3D) &&
    /a\??\.ob\?\.\[i\]/.test(RS));
}
