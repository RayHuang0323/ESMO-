...
//
//  執行：`node tools/browser_check_club_identity_ui.mjs [--headed]`
//  或走 supervisor（硬總時限，見 Browser Harness v1）：
//    `node tools/browser/run-gate.mjs tools/browser_check_club_identity_ui.mjs --timeout 300000`
//
...
//  這支原本自己管 `startDevServer`／`launchChrome`／固定 port／收尾，其中
//  `finally { server?.close?.(); }` 是一個**從未被抓到的 bug**：
//  `startDevServer()` 回傳的物件只有 `.stop`，沒有 `.close`；
//  `server?.close?.()` 對不存在的方法做 optional call，靜默短路成
//  no-op——這支 gate 過去**每一次執行都沒有真的關掉自己的 dev server**。
//  改用 `runGate()` 之後，這一類「收尾呼叫錯方法」的錯誤不再可能發生：
//  cleanup 是 harness 自己做的，gate 只描述測試內容。
// ============================================================================
