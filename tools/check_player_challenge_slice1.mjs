...
const mkChallenge = (opts = {}) => createChallengeInstance({
  ...
  seedSource: 123456, createdAt: 5000, issuedBy: SNAPSHOT_AUTHORITY.id, ...opts,
});
...
const t0 = Date.now();
const run1 = runChallenge({ challenge: ch1.challenge, challengerSnapshot: snapA, defenderSnapshot: snapDef });
...
//  ⑥ 同一場重播：逐值相同
const run2 = runChallenge({ challenge: ch1.challenge, challengerSnapshot: snapA, defenderSnapshot: snapDef });
...
const noLoadoutA = { ...snapA, capturedInputs: snapA.capturedInputs.filter((x) => x !== SNAPSHOT_INPUTS.heroLoadout) };
...
//  ⑦ Slice 1 未涵蓋的輸入誠實標示
ck("⑦ Slice 1 誠實宣告未涵蓋英雄選角 / 原型 / 召喚師技能",
  !snapshotCovers(snapA, "heroPick") && !snapshotCovers(snapA, "archetypes") && !snapshotCovers(snapA, "spells"));
ck("⑦ runner 明說 Slice 1 不呼叫 configureHeroes / Archetypes / Spells",
  /刻意不呼叫[\s\S]{0,6}configureHeroes/.test(runnerSrc)
  && !/eng\.configureHeroes|eng\.configureArchetypes|eng\.configureSpells/.test(code(runnerSrc)));
//  紅方對映
