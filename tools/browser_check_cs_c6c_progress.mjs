...
//
// 執行：`node tools/browser_check_cs_c6c_progress.mjs`
// 或走 supervisor（硬總時限，見 Browser Harness v1）：
//   `node tools/browser/run-gate.mjs tools/browser_check_cs_c6c_progress.mjs --timeout 300000`
//
// ── Browser Harness v1 migration（2026-09-04）──────────────────────────────
// 原本的頂層 `catch (error) { fail += 1; ... }` 把「深處某個 `waitFor`
// timeout」跟「真正的產品斷言失敗」灌進**同一個** `fail` 計數——最後的
// `pass/fail` 摘要完全分不出這一輪紅是環境問題還是產品真的壞了。
// 現在把同一段診斷輸出（失敗時的頁面文字／按鈕清單／存檔片段，對排查很有
// 價值，原樣保留）搬進 `run()` 自己的 try/catch，log 完後 **re-throw**，
// 讓 `runGate()` 判成 `HARNESS_FAIL`，不再混進產品斷言的 tally。
...
const injectSave = async (chrome, url, save, viewport) => {
...
//  ⚠ 這一段完全不碰 chrome／dev server——用假的 `localStorage` shim 直接在
//    Node 裡跑一次真正的 profileStore，決定性地產生 BO3 fixture 存檔，
//    再整包丟進瀏覽器的 localStorage。與 Browser Harness 無關，維持原樣。
let savedJson = null;
...
const result = await runGate({
  name: "C6C browser progress",
  //  ⚠ 這是整個 suite 最重的一支：完整 BO1 練習賽 ＋ BO3 系列賽，跑的是真的
  //    simulation，典型耗時 ~280 秒。用預設上限會在稍慢的一次執行被誤判成
  //    `HARNESS_FAIL`（實測踩過：11/11 產品斷言全過，卻因為軟上限 300 秒到了
  //    而整輪紅）。給它自己的寬裕上限，留足機器變慢時的餘裕。
  timeoutMs: 900_000,
  run: async ({ chrome, url, ck, sleep }) => {
    try {
      const practice = freshSave({ practiceMapKey: "dust2" });
      await injectSave(chrome, url, practice, { width: 390, height: 844, mobile: true });
      ...
      await injectSave(chrome, url, bo3.save, { width: 1280, height: 900, mobile: false });
      ...
      //  ⚠ 診斷輸出跟原本一模一樣——只是現在 log 完會 re-throw，讓 runGate
      //    判成 HARNESS_FAIL，不再把這類錯誤悄悄灌進 `ck()` 的產品 tally。
