...
// ── G8 隔離 ──────────────────────────────────────────────────────────────────
{
  ...
  walk("src");
  ck("G8", "正式流程（useLocalServer／Challenge／GameView…）沒有呼叫 configureItems／toEngineItems／debugSetInventory", callers.length === 0, callers.join(","));
  ...
  ck("G8", "模擬版本閘門綠（仍是 moba-sim.v4：正式輸入不經過裝備層，G1 已證明結果不變）", gateOk && /moba-sim\.v4"/.test(read("src/platform/contracts/simulationVersion.js").match(/MOBA_SIMULATION_VERSION = "[^"]+"/)?.[0] ?? ""), gateOut.split("\n").slice(-4).join(" "));
}
