#!/usr/bin/env node
// ============================================================================
//  tools/shot_milestone_j_close.mjs — Milestone J-close 正式流程驗收
//
...
const OUT_DIR = resolve(ROOT, arg("--out", "review/moba-runtime/milestone-j-close/evidence"));
...
const server = spawn(process.platform === "win32" ? "npm.cmd" : "npm",
  ["run", "preview", "--", "--port", "4185", "--strictPort"], { cwd: ROOT, stdio: "ignore", shell: process.platform === "win32" });
for (let i = 0; i < 120; i++) { try { const r = await fetch(baseUrl, { redirect: "manual" }); if (r.status < 500) break; } catch { /* wait */ } await sleep(500); }
const profileDir = resolve(ROOT, "node_modules/.cache", `esmo-jc-${process.pid}`);
const port = 9800 + (process.pid % 140);
...
try {
  ...
  writeFileSync(resolve(OUT_DIR, "milestone_j_close_browser.json"), JSON.stringify(results, null, 2), "utf8");
