 ...
 *   不是去 map 一個空陣列。體力偏低與否仍由既有的 `isLowEnergy` 判定。
...
const MAKE_EXHAUSTED = `
  ...
  const players = [{ id: "t-tired", name: "測試選手", role: "top", lv: 1, xp: 0,
                     energy: cond.CONDITION.lowEnergyBelow - 1, injuryDays: 3, injured: true }];
  profile.useProfileStore.setState({ players });
  st().save();
  return { exhausted: players.filter((p) => cond.isLowEnergy(p)).length };
`;
    ...
    for (const vp of [
      ...
      const parity = await chrome.evaluate(`
        ...
        const signals =
          ...
          (players.some((p) => cond.isLowEnergy(p)) ? 1 : 0);
      ...
      //  ⚠ Battle Condition UX：體力不再擋出賽 ⇒ 待辦文案改成「安排選手休息」
      //    （點下去直接開體力管理面板）。這裡驗的仍是同一件事：有體力訊號時待辦要浮上來。
      ck(`${vp.label}｜選手體力偏低時，待辦浮上來（空狀態消失）`,
        !tired.empty && /安排選手休息/.test(tired.text),
